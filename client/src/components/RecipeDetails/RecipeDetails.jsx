...
export default function RecipeDetails(props) {
  ...
  let diets =
    recipeDetailDiets[0]?.diets?.join(", ").charAt(0).toUpperCase() +
    recipeDetailDiets[0]?.diets?.join(", ").slice(1);
  let dishTypes =
    recipeDetailDiets[0]?.dishTypes
      ...
      {detail.length ? (
        <div>
          ...
          <div className={styles.containercardDetailsss}>
            <div>
              <h2 className={styles.h2DetailsTitle}>
                {recipeDetail[0]?.title}
              </h2>
            </div>
            <div className={styles.divContainerDishTypesAndDiets}>
              <div className={styles.containerImageDetailsFirst}>
                {recipeDetail[0]?.image ? (
                  <img
                    className={styles.containerImageInside}
                    src={recipeDetail[0]?.image}
                    alt="There is no image"
                  />
                ) : (
                  <img
                    className={styles.imgDetailsDefault}
                    src={photo}
                    alt="There is no image"
                  />
                )}
              ...
              <div>
                <h4 className={styles.divContainerHealthScore}>
                  HEALTH SCORE:<p> {recipeDetail[0]?.healthScore}.</p>
                </h4>
              </div>
              <div className={styles.divContainerDishTypes}>
                <h2>
                  DISH TYPES: <p>{dishTypes}.</p>
                </h2>
              </div>
              <div className={styles.divContainerTypesOfDiets}>
                <h3>
                  TYPES OF DIETS: <p>{diets}.</p>
                </h3>
              </div>
